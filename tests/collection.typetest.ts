...
// Updater function
const updatedFn: Promise<CollectionMemberOf<User>[]> = db
    .where(($) => [$("active"), "?"])
    .update((prev, ctx) => ({ ...prev, age: prev.age + 1 }));
...
const lensUpdateFn: Promise<CollectionMemberOf<User> | undefined> = db.select("abc").update(($) => $("age"), (prev) => prev + 1);
